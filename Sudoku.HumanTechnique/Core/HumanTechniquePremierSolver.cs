    ...
    public class HumanTechniquePremierSolver:ISolverSudoku
    {
        ...
        public SudokuGrid Solve(SudokuGrid s)
        {
            ...
            do
            {
                ...
                if (solved || (!changed && !RunTechnique(p)))
                {
                    break;
                }
            } while (true);
                                            ...
                                            case 3:
                                                {
                                                    ...
                                                    IEnumerable<Cell> nSubset = ((gtTwo[0].Point.Y == gtTwo[1].Point.Y) ? // Same row
                                                            ...
                                                            .Where(c => c.Candidates.ContainsAny(others) && !c.Candidates.ContainsAny(Kermalis.SudokuSolver.Core.Utils.OneToNine.Except(others)));
        ...
        private static bool FindHiddenTuples(Puzzle puzzle, Region region, int amount)
        {
            ...
            bool DoHiddenTuples(int loop, int[] candidates)
            {
                if (loop == amount)
                {
                    ...
                    if (puzzle.ChangeCandidates(cells, Kermalis.SudokuSolver.Core.Utils.OneToNine.Except(candidates)))
                    {
