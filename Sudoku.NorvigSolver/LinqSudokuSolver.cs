...
namespace Sudoku.NorvigSolver
{
    ...
    static class LinqSudokuSolver
    {
        ...
        static LinqSudokuSolver()
        {
             ...
             */
            units =
                (from s in squares from u in unitlist where u.Contains(s) group u by s into g select g).ToDictionary(
                    g => g.Key);
             ...
             *   for s in squares)
             */
            peers = (from s in squares from u in units[s] from s2 in u where s2 != s group s2 by s into g select g)
        ...
        */
        ...
        static Dictionary<string, string> print_board(Dictionary<string, string> values)
        {
            ...
            foreach (var r in rows)
            {
                Console.WriteLine(String.Join("",
                                      (from c in cols
                                          select values["" + r + c].Center(width) + ("36".Contains(c) ? "|" : ""))
                                      .ToArray())
                                  + ("CF".Contains(r) ? line : ""));
            }
        ...
        public static void Test()
        {
            ...
            Console.WriteLine("Solving 'hardest' sodoku took on average " +
                              (DateTime.Now - start).TotalMilliseconds / 300 + " milliseconds, timed over 300 runs");
